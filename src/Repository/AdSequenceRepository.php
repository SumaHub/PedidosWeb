...
class AdSequenceRepository extends ServiceEntityRepository
{
     ...
     * secuencia
     * 
     * @param \App\Entity\AdSequence $sequence Secuencia
    ...
    protected function buildPrefex(String $prefix, Object $entity): String
    {
        if (empty($prefix)) {
            ...
            if (count($prefix) === 4) {
                ...
                $stmt = $connection->prepare("SELECT :field FROM :table WHERE :primary = :value");
                $resultSet = $stmt->executeQuery([
                    'primary' => $prefix[1],
                    'table' => $prefix[2],
                    'field' => $prefix[3],
                    'value' => $entity->$getValueId()
                ]);
